...
namespace ProgressTable.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        public MainWindowViewModel()
        {
            ...
            {
                Serializer<Student[]>.Save("info.dat", students);
            });
            Load = ReactiveCommand.Create(() =>
            {
                Students  = Serializer<Student[]>.Load("info.dat");
                CheckAverage(students);
            });
            Students = new Student[]
            {
                new Student{Name="Кузьменок Денис Витальевич", Visual=2, Architecture=1, Networks=1, Calculate_Math=2, PI=2, Math=0, Electric=2},
                new Student{Name="Прилепа Михаил Сергеевич", Visual=0, Architecture=0, Networks=1, Calculate_Math=1, PI=1, Math=2, Electric=2},
            };
            CheckAverage(students);
